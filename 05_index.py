...
def positions_order():
    ...
    # print(response)
    if response["code"] == 200:
        data["positions_value"] = response["netPositions"][0]["tradeValue"]
        json_object = json.dumps(data, indent=4)
        with open(CONFIDENTIAL_PATH, "w") as outfile:
            outfile.write(json_object)
    else:
...
def custom_message(msg):
    ...
    if msg:
        if FIRST_DATA and formatted_date != data["Today_date"]:
            ...
            data["takeProfit"] = take_profit + price_difference_copy
            data["most_profit_today"] = most_profit_today + price_difference_copy
            data["stopLoss"] = price_difference_copy - stop_loss
            ...
            if data["most_profit_today"] > equity_difference and data["limit"] == "true":
