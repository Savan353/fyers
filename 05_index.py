...
def positions_order():
    ...
    # print(response)
    if response["code"] == 200:
        if len(response["netPositions"]) == 0:
...
def exit_position():
    ...
    """
    data_order = {}
    response = fyers.exit_positions(data=data_order)
    print(response)
        ...
        if FIRST_DATA and formatted_date != data["Today_date"]:
            ...
            while (data["positions_value"] == 0):
                ...
                if data['positions_value'] != 0:
                    ...
                    data["takeProfit"] = round((take_profit + price_difference_copy),3)
                    data["stopLoss"] = round((price_difference_copy - stop_loss),3)
                    json_object = json.dumps(data, indent=4)
                    with open(CONFIDENTIAL_PATH, "w") as outfile:
                        outfile.write(json_object)
            ...
            #     if price_difference_copy > data["takeProfit"] - LIMIT:
            #         data["takeProfit"] = data["takeProfit"] + LIMIT
            #         data["stopLoss"] = data["stopLoss"] + LIMIT
            #         json_object = json.dumps(data, indent=4)
            #         with open(CONFIDENTIAL_PATH, "w") as outfile:
            #             outfile.write(json_object)
